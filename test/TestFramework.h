...
namespace Test
{
  ...
  inline bool AreEqual(char *actual, char *expected)
  {
    return AreEqual(static_cast<const char *>(actual),
                    static_cast<const char *>(expected));
  }

  inline bool AreEqual(char *actual, const char *expected)
  {
    return AreEqual(static_cast<const char *>(actual), expected);
  }

  inline bool AreEqual(const char *actual, char *expected)
  {
    return AreEqual(actual, static_cast<const char *>(expected));
  }

  template <typename A, typename B>
  inline bool AreEqual(const A &actual, const B &expected)
  {
    return actual == expected;
  }
  ...
  template <typename A, typename B>
  inline void CheckNe(const A &actual, const B &expected, const char *actualExpr,
                      const char *expectedExpr, const char *file, int line)
  {
    if (AreEqual(actual, expected))
    {
  ...
  template <typename A, typename B>
  inline void CheckLt(const A &actual, const B &expected, const char *actualExpr,
                      const char *expectedExpr, const char *file, int line)
  {
    if (!(actual < expected))
    {
  ...
  template <typename A, typename B>
  inline void CheckGt(const A &actual, const B &expected, const char *actualExpr,
                      const char *expectedExpr, const char *file, int line)
  {
    if (!(actual > expected))
    {
  ...
  template <typename A, typename B>
  inline void CheckLe(const A &actual, const B &expected, const char *actualExpr,
                      const char *expectedExpr, const char *file, int line)
  {
    if (!(actual <= expected))
    {
  ...
  template <typename A, typename B>
  inline void CheckGe(const A &actual, const B &expected, const char *actualExpr,
                      const char *expectedExpr, const char *file, int line)
  {
    if (!(actual >= expected))
    {
  ...
  inline void CheckInRange(const T &actual, const L &lower, const U &upper,
                           const char *actualExpr, const char *lowerExpr,
                           const char *upperExpr, const char *macroName,
  ...
  inline void Check(bool condition, const char *expr, const char *file,
                    int line)
  {
    if (!condition)
    {
...
#define CHECK_CONTAINS(actual, lower, upper)                              \
  Test::CheckInRange((actual), (lower), (upper), #actual, #lower, #upper, \
                     "CHECK_CONTAINS", __FILE__, __LINE__)
#define CHECK_IN_BOUNDS(actual, lower, upper)                             \
  Test::CheckInRange((actual), (lower), (upper), #actual, #lower, #upper, \
                     "CHECK_IN_BOUNDS", __FILE__, __LINE__)
#define CHECK_IN_RANGE(actual, lower, upper)                              \
  Test::CheckInRange((actual), (lower), (upper), #actual, #lower, #upper, \
                     "CHECK_IN_RANGE", __FILE__, __LINE__)
