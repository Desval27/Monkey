...
TEST_CASE("PitchEngine returns root frequency without a temperament")
{
  ...
  CheckNear(engine.frequency_from_scale_index(7), music::BASE_HZ);
  CheckNear(engine.get_frequency(music::TemperedPitch(7, 2, 50.0f)),
            music::BASE_HZ);

  engine.set_root_hz(123.45f);
  CheckNear(engine.frequency_from_scale_index(7), 123.45f);
  CheckNear(engine.get_frequency(music::TemperedPitch(7, 2, 50.0f)), 123.45f);
}
          ...
          "scale map")
{
  ...
  CheckNear(engine.frequency_from_scale_index(7, 1),
            temperament.frequency_from_root(110.0f, Degree(7), 1));

  CheckNear(engine.frequency_from_scale_index(4, -1, 25.0f),
...
TEST_CASE("PitchEngine maps scale indices through the active scale map")
{
  ...
  CheckNear(engine.frequency_from_scale_index(0),
            temperament.frequency_from_root(220.0f, Degree(0), 0));

  CheckNear(engine.frequency_from_scale_index(4, 1),
            temperament.frequency_from_root(220.0f, Degree(9), 1));

  CheckNear(engine.frequency_from_scale_index(5),
            temperament.frequency_from_root(220.0f, Degree(0), 1));

  CheckNear(engine.frequency_from_scale_index(8, -1, -12.5f),
          ...
          "index period")
{
  ...
  CheckNear(engine.frequency_from_weighted_scale_index(8, 0.75f, thirdOnly),
            temperament.frequency_from_root(220.0f, Degree(4), 1));

  CheckNear(
    engine.frequency_from_weighted_scale_index(8, 0.75f, thirdOnly, 1, 10.0f),
...
TEST_CASE("PitchEngine random shaping biases weighted lookup")
{
  ...
  CheckNear(
    engine.frequency_from_weighted_scale_index(
...
TEST_CASE("PitchEngine shapes unit random values within half-open bounds")
{
  ...
  CheckNear(PitchEngine<>::shape_unit_random(-0.5f, RandomShape::Uniform),
            0.0f);
  CheckNear(PitchEngine<>::shape_unit_random(1.0f, RandomShape::Uniform),
            0.999999f);
  CheckNear(PitchEngine<>::shape_unit_random(0.5f, RandomShape::BiasedLow),
            0.25f);
  CheckNear(PitchEngine<>::shape_unit_random(0.5f, RandomShape::BiasedHigh),
            0.75f);
}
...
TEST_CASE("PitchEngine converts tempered pitches with degree period and cents")
{
  ...
  CheckNear(engine.get_frequency(pitch),
