...
static void CheckNear(float actual, float expected, float tolerance = 0.001f)
{
...
TEST_CASE("PitchEngine uses scale indices as temperament degrees without a scale map")
{
...
TEST_CASE("PitchEngine maps scale indices through the active scale map")
{
  ...
  scale.SetDegrees(majorPentatonic);
...
TEST_CASE("PitchEngine weighted lookup selects mapped degrees and keeps index period")
{
  Temperament<> temperament;
  ScaleMap<Music::PENTATONIC> scale;
  const Music::DegreeMap<Music::PENTATONIC> majorPentatonic = {0, 2, 4, 7, 9};
  ...
  CheckNear(engine.FrequencyFromWeightedScaleIndex(8, 0.75f, thirdOnly, 1,
                                                  10.0f),
...
TEST_CASE("PitchEngine random shaping biases weighted lookup")
{
  ...
  CheckNear(engine.FrequencyFromWeightedScaleIndex(
                0, 0.5f, PentatonicEngine::RandomShape::BiasedLow,
                uniformWeights),
            temperament.FrequencyFromRoot(220.0f, Degree(2), 0));

  CheckNear(engine.FrequencyFromWeightedScaleIndex(
                0, 0.5f, PentatonicEngine::RandomShape::BiasedHigh,
                uniformWeights),
