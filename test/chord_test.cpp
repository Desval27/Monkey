...
static void CheckChordName(const ChordEvent<> &chord,
...
TEST_CASE("Chord names reflect raised-scale tones when the triad changes quality") {
  ...
  scale.SetDegrees(harmonicMinor);
...
TEST_CASE("Chord extensions add scale-stacked chord tones") {
  ...
  const std::size_t toneCount = chord.GetChordTones(scale, 12, tones, 6);
...
TEST_CASE("Chord alterations adjust generated chord tones and names") {
  ...
  const std::size_t toneCount = chord.GetChordTones(scale, 12, tones, 5);
...
TEST_CASE("Chord sus extensions replace the third") {
  ...
  const std::size_t toneCount = chord.GetChordTones(scale, 12, tones, 3);
...
TEST_CASE("Chord inversion rotates chord tones by period") {
  ...
  const ChordEvent chord(0, NoteValue::Whole, ChordExtension::Seventh,
                         ChordAlteration::None, 1);
  const std::size_t toneCount = chord.GetChordTones(scale, 12, tones, 4);
...
TEST_CASE("Chord naming uses the temperament period when provided") {
  ...
  scale.SetDegrees(Music::IONIAN_D15);
  ...
  const std::size_t toneCount = chord.GetChordTones(
