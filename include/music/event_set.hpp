...
struct EventTraits<NoteEvent>
{
  ...
  static bool is_hit(const NoteEvent& event) { return event.note != REST; }
};
...
struct EventTraits<ChordEvent<MAX_DEGREES, SCALE_DEGREES>>
{
  ...
  static const Note& pitch(const ChordEvent<MAX_DEGREES, SCALE_DEGREES>& event)
  {
    return event.root;
  }
  ...
  static const NoteValue& value(
    const ChordEvent<MAX_DEGREES, SCALE_DEGREES>& event)
  {
    return event.value;
  }
  static bool is_hit(const ChordEvent<MAX_DEGREES, SCALE_DEGREES>& event)
  {
...
{
  static bool is_hit(const bool& event) { return event; }
};

/**
 * @brief A generic container for musical events, such as notes or chords, that
 * occur at specific time intervals. This class provides functionality to
 * manage a sequence of events, including adding new events, calculating event
 * density, and determining event positions based on time signatures. It is
 * designed to be flexible and can be specialized for different types of
 * musical events by defining appropriate traits. The EventSet class is
 * intended to be used as a base for specific event types, such as NoteEventSet
 * and ChordEventSet, which can be defined using type aliases with specific
 * event types and maximum capacities.
 */
...
public:
   ...
   */
  [[nodiscard]] bool at_capacity() const { return size() >= capacity() - 1; }
   ...
   */
  [[nodiscard]] int get_bars_for_time_signature(const TimeSignature& ts) const
  {
    int ppb = ts.get_pulses_per_bar();
    if (ppb != 0) {
      return get_total_event_pulses() / ppb;
    }
    return 0;
  }
   ...
   * set based on its index. This function iterates through the events up to
   * the specified index, summing their durations (in pulses) to determine the
   ...
   * calculating bars based on time signatures or determining event positions
   * in a timeline.
   ...
   * pulse position in the timeline. This function first checks if the event
   * set is empty or if the pulse value is negative, in which case it returns
   * -1 to indicate an invalid index. It then normalizes the pulse value by
   * taking it modulo the total number of pulses represented by all events,
   * allowing for cyclic behavior. The function iterates through the events,
   * keeping a running total of pulses, and returns the index of the event
   * where the normalized pulse falls within its duration. If the pulse exceeds
   * the total duration of all events, it returns the index of the last event.
   ...
   */
  [[nodiscard]] int get_event_index_for_pulse(int pulse) const
  {
    if (size() == 0 || pulse < 0) {
      return -1;
    }
  ...
  /**
   * @brief Retrieves the event that corresponds to a specific pulse position
   * in the timeline. This function uses the get_event_index_for_pulse method to
   ...
   * normalized to fit within the total duration of the events in the set.
   * @return const T& A reference to the event that corresponds to the
   ...
   * set. This function returns a pointer to the first event in the array,
   * allowing for iteration or manipulation of the events directly. It is
   * important to note that the caller should ensure that they do not access
   * beyond the current count of events, as this could lead to undefined
   * behavior. The const version of this function allows for read-only access
   * to the events, ensuring that the caller cannot modify the event data when
   ...
   * bounds of the current event count. It is important to ensure that the
   * index provided is valid (i.e., less than the current count of events) to
   * avoid undefined behavior. The const version of this operator allows for
   * read-only access to the events, ensuring that the caller cannot modify the
   * event data when using a const reference to the EventSet.
   ...
   * for retrieval of the event data. The caller should ensure that the index
   * is valid to avoid undefined behavior. The const version of this operator
   ...
   * function checks if the current event count is less than the defined
   * maximum capacity before adding the new event. If the event is successfully
   * added, it increments the event count and returns true; otherwise, it
   * returns false to indicate that the event could not be added due to
   * reaching capacity. The caller should ensure that the event being added is
   * valid and that the set has not already reached its maximum capacity to
   * avoid failed additions. The emplace function provides a similar
   * functionality but allows for constructing the event in place using
   * variadic arguments, which can be more efficient for certain types of
   * events.
   ...
   * caller should ensure that the arguments provided are valid for
   * constructing the event type and that the set has not already reached its
   * maximum capacity to avoid failed additions.
   ...
   * type and that the set has not already reached its maximum capacity to
   * avoid failed additions.
  ...
  /**
   * @brief Calculates the density of events in the set, defined as the ratio
   * of the number of events that are considered "hits" (i.e., events that have
   * a meaningful value or state) to the total number of events in the set.
   * This function iterates through all events in the set, counting how many
   * are hits based on the is_hit trait defined for the event type. The density
   * is returned as a floating-point value between 0 and 1, where 0 indicates
   * no hits and 1 indicates that all events are hits. This metric can be
   * useful for analyzing the distribution of events in a musical sequence and
  ...
  /**
   * @brief Provides access to the pitch and value of events in the set based
   * on their index. These functions use the EventTraits defined for the event
   * type to retrieve the pitch and value of the event at the specified index.
   * The caller should ensure that the index provided is valid (i.e., less than
   * the current count of events) to avoid undefined behavior. The is_hit
   * function allows for checking if a specific event at a given index is
   * considered a hit based on its traits, which can be useful for analyzing
   * the events in the set and understanding their significance within a
   * musical sequence.
   ...
   * ensure valid access.
   * @return Note& A reference to the pitch of the event at the specified
   * index, allowing for retrieval or modification of the pitch data. The
   * caller should ensure that the index is valid to avoid undefined behavior.
   * The const version of this function returns a const reference, ensuring
   * read-only access to the pitch data when using a const reference to the
   * EventSet.
   * @return NoteValue& A reference to the value of the event at the specified
   * index, allowing for retrieval or modification of the value data. The
   * caller should ensure that the index is valid to avoid undefined behavior.
   * The const version of this function returns a const reference, ensuring
   ...
   * the events in the set and understanding their significance within a
   * musical sequence.
   * @param index The index of the event for which to check if it is a hit.
   * This value should be less than the current count of events to ensure valid
   * access.
   * @return bool True if the event at the specified index is considered a hit
   * based on its traits, false otherwise. The caller should ensure that the
   * index is valid to avoid undefined behavior.
 ...
 * template with specific event types and maximum capacities. These type
 * aliases provide convenient names for commonly used event sets in musical
 * applications, allowing for easier readability and usage in code that manages
 * musical events. The PatternEventSet is a simple boolean event set that can
 * be used for patterns or rhythms, while the NoteEventSet and ChordEventSet
 * are designed to hold more complex musical events with pitch and value
 * information. The caller should choose the appropriate event set type based
 * on the specific requirements of their musical application.
 */
