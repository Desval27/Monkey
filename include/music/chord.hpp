...
struct ChordEvent
{
                              ...
                              std::size_t noteLen) const
  {
    ...
    apply_inversion(note, toneCount, degreesPerPeriod);
                      ...
                      MString<N>& out) const
  {
    ...
    if (!build_chord_name_from_quality(rootIdx, quality, out)) {
      return false;
    }
...
private:
                                          ...
                                          MString<N>& out)
  {
    ...
    switch (quality) {
      default: {
        if (!build_chord_name_from_quality(scaleIndex, quality, out)) {
          return false;
        }
        return append_extension_name(out, extensions) &&
               append_alteration_name(out, alterations);
  ...
  void apply_inversion(Note note[],
                       std::size_t toneCount,
    ...
    int degreesPerPeriod)
  {
    ...
    for (std::size_t i = 0; i < scale.size() && i < SCALE_CHORD_COUNT; ++i) {
      thirdIntervals[thirdCount++] =
        get_scale_span_interval(scale, i, 2, degreesPerPeriod);
      fifthIntervals[fifthCount++] =
        get_scale_span_interval(scale, i, 4, degreesPerPeriod);
    }
  ...
  template<std::size_t N>
  static bool build_chord_name_from_quality(int scaleIndex,
                                            ChordQuality quality,
                                            MString<N>& out)
  {
    switch (quality) {
      case ChordQuality::Major:
        return out.set(roman_numeral_for_scale_index(scaleIndex));
      case ChordQuality::Minor:
        return out.set(lowercase_roman_numeral_for_scale_index(scaleIndex));
  ...
  static int min_value(const int values[], std::size_t count)
  {
  ...
  static int max_value(const int values[], std::size_t count)
  {
    if ((values == nullptr) || count == 0) {
      return 0;
    }
  ...
  static int greatest_value_less_than(const int values[],
                                      std::size_t count,
                                      int limit)
  {
    int best = 0;
  ...
  static int least_value_greater_than(const int values[],
                                      std::size_t count,
                                      int limit)
  {
    int best = 0;
  ...
  static const char* roman_numeral_for_scale_index(int scaleIndex)
  {
  ...
  static const char* lowercase_roman_numeral_for_scale_index(int scaleIndex)
  {
    switch (wrap(scaleIndex, SCALE_CHORD_COUNT)) {
      case 0:
        return "i";
