...
static inline char *tuning_copy_string(char *dst, const char *src,
                                       std::size_t maxLen) {
  if (!dst || maxLen == 0)
    return dst;
  if (!src) {
 ...
 * octave. This is a common calculation in music theory for expressing intervals
 * in a logarithmic scale. The function takes a positive frequency ratio as
 * input and returns the corresponding number of cents, calculated using the
 * formula: cents = 1200 * log2(ratio). If the input ratio is zero or negative,
 * the function returns 0.0f to avoid invalid logarithm calculations.
 * @param ratio The frequency ratio to convert, where a ratio of 1.0 corresponds
 * to unison, 2.0 corresponds to an octave, and values between 1.0 and 2.0
 * correspond to intervals within the octave.
 * @return float The number of cents corresponding to the given frequency ratio,
 * calculated as 1200 times the base-2 logarithm of the ratio.
 */
...
/**
 * @brief Converts a BPM value to a frequency in Hz using a default PPQN (Pulses
 * Per Quarter Note) resolution defined by the constant PPQN. This is a
 ...
 * default PPQN
 */
static inline float BpmToFreq(float bpm) { return BpmToFreq(bpm, 1); }
