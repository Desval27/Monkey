...
namespace Music
{
                                          ...
                                          ChordEventSet<MAX_DEGREES, SCALE_DEGREES, MAX_EVENTS> &chords)
  {
    ...
    for (std::size_t i = 0; i < chordsToEmit; ++i)
    {
      ...
      ChordExtension extensions =
          (degree == ScaleDegree::V) ? ChordExtension::Seventh
                                        ...
                                        NoteEventSet<MAX_EVENTS> &events)
  {
    ...
    for (std::size_t i = 0; i < pattern.Count() && !events.AtCapacity();
         i++, pulses = pulses + granularity, chordPulses = chordPulses + granularity)
    {
                                 ...
                                 const NoteEventSet<MAX_EVENTS> &events)
  {
    ...
    const int phraseStartPct =
        phraseCount > 0 ? static_cast<int>((consonantPhraseStarts * 100UL) / phraseCount) : 0;
    const int phraseEndPct =
        phraseCount > 0 ? static_cast<int>((consonantPhraseEnds * 100UL) / phraseCount) : 0;
    const int repeatPct =
        transitionCount > 0
            ...
            : 0;
    const int stepPct =
        transitionCount > 0 ? static_cast<int>((stepMoves * 100UL) / transitionCount) : 0;
    const int hugeLeapPct =
        transitionCount > 0 ? static_cast<int>((hugeLeaps * 100UL) / transitionCount) : 0;
    const int phraseDensity =
        pitchedEventCount > 0 ? static_cast<int>((phraseCount * 100UL) / pitchedEventCount) : 0;
                       ...
                       const NoteEventSet<MAX_EVENTS> &events)
  {
    ...
    for (std::size_t i = 0; i < events.Count(); i++)
    {
      ...
      std::cout << std::setw(5) << std::right << noteLabel << "-"
                << std::setw(10) << std::left << GetNoteValueText(events[i].value);
  ...
  template <std::size_t MAX_DEGREES, std::size_t SCALE_DEGREES, std::size_t MAX_EVENTS>
  void DebugChordEvents(const TimeSignature &ts,
                        const Temperament<MAX_DEGREES> &t,
